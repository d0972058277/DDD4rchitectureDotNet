...
public class GrpcCorrelationClientInterceptor : Interceptor
{
    ...
    {
        _correlationPropagator =
            correlationPropagator ?? throw new ArgumentNullException(nameof(correlationPropagator));
    }
...
public class GrpcCorrelationServerInterceptor : Interceptor
{
    ...
    private void SetupCorrelationContext(ServerCallContext context)
    {
        ...
        if (!string.IsNullOrEmpty(correlationId)) _correlationContextFactory.Create(correlationId, "grpc-metadata");
    }
}
#endif